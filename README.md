...
- **密钥备份与恢复**（`Ctrl+G` → ⑤）：把 `~/.ssh` 下的 `id_*`（私钥+公钥）、`config`、`known_hosts`
  ...
  每次推送公钥前还会自动回传一份远端 `authorized_keys` 快照，误删也能回滚
- **远端公钥盘点**（`Ctrl+G` → ⑥）：并发拉取各主机的 `authorized_keys` 与本机密钥比对，
  按「算法+密钥体」判定**已授权 / 缺失（待推送）/ 废弃（本机无对应私钥）**，并给出汇总
- **密钥轮换**（`Ctrl+G` → ⑦）：生成新密钥 → 分发到目标主机 → 用新私钥验证可登录 → 移除旧密钥；
  验证失败的主机保留旧密钥，结果页逐台列出三个阶段（分发 / 验证 / 移除）
...
- **远端文件浏览器**（`Ctrl+O`）：基于 SFTP 浏览服务器文件系统，支持进入/返回目录、过滤；
  ...
  `U` 上传本地文件、`D` 删除（目录递归、二次确认）；**目录会递归传输**（`d` 下载整棵子树 / `U` 上传整个本地目录，保留子目录结构）
...
- **多主机批量执行**（`batch` 元命令）：选多台目标（或 `batch <关键词>` 按名称/主机/分组过滤），
...
| `Ctrl+G` | SSH 密钥管理：生成密钥 / 推送公钥 / 查看公钥 / 打标签（`#prod` 等） |
...
| `batch` | 多主机批量执行：先选目标（空格多选），再输入命令，并发执行并汇总输出 |
| `batch <关键词>` | 跳过选择，直接对「名称/主机/用户/分组」匹配的连接批量执行（如 `batch prod`） |
| `ping` | 连接健康巡检：对匹配连接并发检查连通性，区分「可达 / 认证失败 / 不可达 / 超时」 |
| `ping <关键词>` | 只巡检匹配的连接（如 `ping prod`） |
...
- ✔ **远端公钥盘点**（`Ctrl+G` → ⑥）：连上主机后拉取 `authorized_keys`，与本机密钥指纹比对，
  一眼看出「哪台还没授权」「哪台有已废弃的旧密钥」——这是最实用的一项审计能力
- **known_hosts 管理**：✔ 已实现——首次连接交互式确认指纹并落盘，指纹不符直接拒绝；见上「安全」一节
- ✔ **密钥轮换**（`Ctrl+G` → ⑦）：生成新密钥 → 批量分发 → **用新私钥验证确实能登录** → 再移除旧指纹，
  全流程自动化；任一主机验证失败即**保留其旧密钥**，绝不会把自己锁在门外

## 已知限制（密钥相关）

- 标签索引（`~/.sshtool/key-tags.json`）与密钥备份 / 恢复均已实现（见上一节）；备份口令无找回机制，请另行保管
...
5. 双向传输与维护（对话框底部有提示，操作结果在状态栏反馈）：
   ...
   - **`d`** 下载到本地：输入本地路径并回车（默认填文件名，相对本地当前工作目录）；选中目录则**递归下载整棵子树**
   - **`U`** 上传本地文件：输入本地文件路径并回车，上传到当前远端目录（同名覆盖）；输入**本地目录**则**递归上传整棵子树**
